...
#		print STDERR "No cell: $row_min, $col\n";
	    ...
	    if ($value =~ /\((\d+\.\d+)\)/) {
		print STDERR "Found potential prefix $1 at $row,$col.\n";
		if ($fig_prefix) {
		    print STDERR "!!!PROBLEM: already have FIG prefix $fig_prefix for row $row.\n";
		} else {
		    $fig_prefix = $1;
		}
		$outSheet->write($row, $col, $cell->value, $format);
	    } elsif ($value =~ /^(\s*\d+\s*\,?)+\s*$/) {
		$value =~ s/^\s*//;
		$value =~ s/\s*$//;
		my @idxs = split/\s*\,\s*/, $value; 
		my @locus;
		foreach my $i (@idxs) {
#		    print STDERR "\tFound potential index $i at $row,$col\n";
		    my $fig_id = "$fig_prefix.peg.$i";
		    my $fid = get_feature_id_by_accession($dbh, $fig_id);
		    my $locusr = get_locus_tag_by_feature_id($dbh, $fid);
		    if (!@$locusr) {
			print STDERR "!!!!PROBLEM: Couldn't find locus for $fig_id.\n";
			push @locus, $i;
		    } else {
			push @locus, @$locusr;
		    }
