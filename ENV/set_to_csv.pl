...
foreach my $seqid (sort {$a<=>$b} @$seq_ids) {
    ...
    my $contig = "\"" . join("\s",@{&get_accession_by_seq_id($dbh, $seqid)}) . "\"";
    ...
    foreach my $featid ( sort { $feat_ref->{$a}->{'location'}->{$seqid}->{'feat_min'} <=>
		...
		my $primary_acc;
		if ($acc_source) {
			$primary_acc = $feat_ref->{$featid}->{'accessions'}->{$acc_source}->[0]
		}
		...
		$RAST_ann = $feat_ref->{$featid}->{'annotation'}->{4}->{'RAST'} if (defined $feat_ref->{$featid}->{'annotation'}->{4}->{'RAST'});
		...
		$output .= "\t$aa";
		$output .= "\t$subsystem";
		$output .= "\t\"" . join("::", (keys %{$role->{'main'}})) . "\"";
		$output .= "\t\"" . join("::", (keys %{$role->{'sub'}})) . "\"";
		$output .= "\t$COG_cat";
		$output .= "\t$SEED_subsys";
		$output .= "\t\"" . join(" ", (keys %$TC)) . "\"";
		$output .= "\t\"" . join(" ", (keys %KO)) . "\"";
		...
		$output .= "\t$COG_desc";
		$output .= "\t\"" . join(" ", (keys %PF)) . "\"";
		$output .= "\t\"" . join("::", (keys %PFdesc)) . "\"";
		$output .= "\t\"" . join(" ", (keys %TIGR)) . "\"";
		$output .= "\t\"" . join("::", (keys %TIGRdesc)) . "\"";
		$output .= "\t$SP";
		$output .= "\t$TMH";
		$output .= "\t\"" . join("::", (keys %{$cofactor->{'ec'}})) . "\"";
		$output .= "\t\"" . join("::", (keys %{$cofactor->{'ev'}})) . "\"";
		$output .= "\t\"" . join("::", (keys %$CAZY)) . "\"";
		$output .= "\t\"" . join("::", (keys %$MEROPS)) . "\"";
		$output .= "\n";
    }
}
...
print $OUT #&header(-type => 'text/plain',
     ...
     "Internal id\tprimary_acc\t"
	 . "Feat_type\tContig\tStart\tStop\tStrand\tAA"
	 . "\tSubsystem\tMainrole\tSubrole\tCOG Cat\tRAST subsystem"
	 . "\tTC\tKO\tKO description\tRAST product descriptor\tIMG product descriptor"
	 . "\tEC\tCOG\tCOG description\tPfam\tPfam description\t"
	 . "TIGRfam\tTIGRfam description\t"
	 . "SP\tTMH\tEC_Cofactor\tev_Cofactor\tCAZy\tMEROPS\n";
print $OUT $output;
