...
# This generates tbl files suitable for input into tbl2asn for building Genbank WGS submission files
...
my $set_name = $args{'n'};
my $set_id = $args{'i'};
if ($set_name && ! $set_id) { $set_id = set_name_to_id($dbh, $set_name); }
...
foreach my $sid (sort { $seqs_ref->{$b}->{seq_length} <=> $seqs_ref->{$a}->{seq_length} } keys %$seqs_ref) {
    ...
    foreach my $fid (sort { $features_ref->{$a}->{'location'}->{$sid}->{'feat_min'} <=> 
				$features_ref->{$b}->{'location'}->{$sid}->{'feat_min'} ||
				$features_ref->{$a}->{'location'}->{$sid}->{'feat_max'} <=> 
				$features_ref->{$b}->{'location'}->{$sid}->{'feat_max'} } keys %$features_ref) {
		my $ref = $features_ref->{$fid};

		# should not be possible because we're pulling the features by $sid
	...
	#	}
		...
		my ($end5, $end3);
		if ($ref->{'location'}->{$sid}->{'strand'} < 0) {
			$end5 = $ref->{'location'}->{$sid}->{feat_max};
			if ($ref->{'location'}->{$sid}->{'max_partial'}) {
				$end5 = "<" . $end5;
			}
			$end3 = $ref->{'location'}->{$sid}->{feat_min}; 
			if ($ref->{'location'}->{$sid}->{'min_partial'}) {
				$end3 = ">" . $end3;
			}
		} else {
			$end5 = $ref->{'location'}->{$sid}->{feat_min};
			if ($ref->{'location'}->{$sid}->{'min_partial'}) {
				$end5 = "<" . $end5;
			}
			$end3 = $ref->{'location'}->{$sid}->{feat_max};
			if ($ref->{'location'}->{$sid}->{'max_partial'}) {
				$end3 = ">" . $end3;
			}
		}
