...
if (! $set_id) {
    ...
    $set_id = load_sequence_sets($dbh, $prefix, $organism, 0);
}
...
# Run prokka
################################################################################
my $prokka_gff = $prefix . ".gff";
if (! -e "${prokka_gff}.LOADED") {
    my $prokka_fsa = $prefix . ".fsa";
    my $prokka_faa = $prefix . ".faa";
	...
	my $prokka_cmd = "$PROKKA --prefix $prefix --locustag $prefix $prokka_org $prokka_gram --cpus $cpus --force $genome_fasta";
	print "$prokka_cmd\n";
    ...
    else { die "Need to provide a protein fasta\n" }
    # Load prokka
...
if (! -e "${prefix}.TIGRfam.domtblout.LOADED") {
    # Run TIGRfams
    my $TIGRfam_err = system("$HMMSEARCH -o $prefix.TIGRfam.out --tblout $prefix.TIGRfam.tblout --domtblout $prefix.TIGRfam.domtblout --noali --acc --cut_nc --cpu $cpus $TIGRFAMDB $protein_faa");
    if ($TIGRfam_err) { die "Bad TIGRfam command\n" }

    # Load TIGRfams
    my $load_TIGRfam_err = system("$ENV{ENVSCRIPTS}/load_hmms_to_ENV.pl -D $db -U $user -P $pswd -i $prefix.TIGRfam.domtblout");
    if ($load_TIGRfam_err) { die "Bad load TIGRfams command\n" }
    else { system("touch ${prefix}.TIGRfam.domtblout.LOADED") }

    my $ann_by_hmm_err = system("$ENV{ENVSCRIPTS}/ann_by_hmm.pl -D $db -u $user -p $pswd -s $set_id");
}

if (! -e "${prefix}.Pfam.domtblout.LOADED") {
    # Run Pfam
    my $Pfam_err = system("$HMMSEARCH -o $prefix.Pfam.out --tblout $prefix.Pfam.tblout --domtblout $prefix.Pfam.domtblout --noali --acc --cut_ga --cpu $cpus $PFAMDB $protein_faa");
    if ($Pfam_err) { die "Bad Pfam command\n" }

    # Load Pfam
    my $load_Pfam_err = system("$ENV{ENVSCRIPTS}/load_hmms_to_ENV.pl -D $db -U $user -P $pswd -i $prefix.Pfam.domtblout");
    if ($load_Pfam_err) { die "Bad load pfams command\n" }
    else { system("touch ${prefix}.Pfam.domtblout.LOADED") }
}

if (! -e "${prefix}.KOfamscan.out.LOADED") {
    # run KOfams
    my $KOfamscan_err = system("$HMMSEARCH -o $prefix.KOfam.out --tblout $prefix.KOfam.tblout --domtblout $prefix.KOfam.domtblout --noali --acc -E 1e-25 --cpu $cpus $KOFAMDB $protein_faa");
    if ($KOfamscan_err) { die "Bad KOfamscan command\n" }

    # load KOfamscan
    my $load_KO_err = system("$ENV{ENVSCRIPTS}/load_KOfamscan.pl -D $db -u $user -p $pswd -i $prefix.KOfam.domtblout");
    if ($load_KO_err) { die "Bad load_KOfamscan command\n" }
    else { system("touch ${prefix}.KOfamscan.out.LOADED") }
}
...
if (! -e "signalp.LOADED" && -e "./dbCAN_out/signalp.out") {
    ...
    else { system("touch ${prefix}.signalp.LOADED") }
}

if (! -e "${prefix}.tmhmm.LOADED") {
    # Run tmhmm
    my $tmhmm_err = system("$TMHMM $protein_faa > $prefix.tmhmm");
    if ($tmhmm_err) { die "Bad tmhmm command\n" }

    # Load tmhmm
    my $load_tmhmm_err = system("$ENV{ENVSCRIPTS}/load_tmhmm.pl -D $db -u $user -p $pswd -i $prefix.tmhmm");
    if ($load_tmhmm_err) { die "Bad load_tmhmm command\n" }
    else { system("touch ${prefix}.tmhmm.LOADED") }
}

if (! -e "${prefix}.psortb.LOADED") {
    # Run psortb
    my $psort_gram = "";
    if ($gram) {
    ...
    my $psort_err = system("$PSORT $psort_gram $psort_arch -o terse $protein_faa > $prefix.psortb");
    if ($psort_err) { die "Bad psort command\n" }

    # Load psortb
    my $load_psort_err = system("$ENV{ENVSCRIPTS}/load_psort.pl -D $db -u $user -p $pswd -i $prefix.psortb");
    if ($load_psort_err) { die "Bad load_psort.pl command\n" }
    else { system("touch ${prefix}.psortb.LOADED") }
}
