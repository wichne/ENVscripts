...
FEATURE:while (my $feature = $gffio->next_feature()) {
    ...
    if ($seq_acc) {
		...
		if (!$seqobj) {
			print STDERR "Sequence '$seq_acc' is not in database ($seqobj). Skipping features associated with this accession\n";
			next FEATURE;
		}
		$seq_id = $seqobj->id;
    } else {
...
#	else { $asmbl_id = $res->[0]->[0] }
    ...
    foreach my $tag (@tags) {
		...
		# added the 'seq_id' part because of data coming from the HiC sequencing center (Phase Genomics)
		if (($tag eq "ID" || $tag eq "seq_id") && !$locus_tag) {
    ...
    if (! $feat_type || ! $locus_tag) { die "Bad data feat_type $feat_type locus_tag $locus_tag\n" }
    print STDERR "Doing $feat_type $locus_tag...\n";
    # look for existing feature
    # first by accession...
    if ( ! $min || ! $max || ! $strand || ! $seq_id) { die "Bad data: min $min max $max strand $strand on seq_id $seq_id\n" }
    ...
    if (@$feat_r) {
		...
		if (grep /^product$/, @tags) {
			my $product = uri_unescape([$feature->get_tag_values('product')]->[0]);
			$ann_h->execute($feat_id, 66, $product, $source, 10);
		} elsif (grep /^Name$/, @tags) {
			my $product = uri_unescape([$feature->get_tag_values('Name')]->[0]);
			$ann_h->execute($feat_id, 66, $product, $source, 10);
			}
		...
		if (grep /^EC$/, @tags) {
			foreach my $ec (@{$feature->get_tag_values('EC')}) {
				$ann_h->execute($feat_id, 1, $ec, $source, 10);
			}
		}

		if (grep /^sso$/i, @tags) { # KBase SEED sequence ortholog
			$ann_h->execute($feat_id, 23, [$feature->get_tag_values('sso')]->[0], $source, 10);
		}
   }
}
